...
# ИСПРАВЛЕННЫЕ ИМПОРТЫ - теперь корректные
...
class ClientDialog(tk.Toplevel):
    def __init__(self, parent, title="Добавить клиента", client: Client=None):
        ...
        tk.Label(self, text="Вес груза (тонны):").grid(row=1, column=0, sticky='e', padx=6, pady=6)
    ...
    def on_save(self):
        ...
        if not ok_weight:
            messagebox.showwarning("Неверный вес", "Вес должен быть положительным числом не более 10000 тонн.")
            self.weight_var.set('')
            return
...
class VehicleDialog(tk.Toplevel):
    def __init__(self, parent, title='Добавить транспорт', vehicle: Vehicle=None):
        ...
        tk.Label(self, text="Вместимость (тонны):").grid(row=1, column=0, sticky='e', padx=6, pady=6)
...
class TransportApp(tk.Tk):
    def __init__(self):
        ...
        # ИСПРАВЛЕНО: правильное создание TransportCompany
    ...
    def create_tables(self):
        ...
        for col, text in [('name','Имя'),('weight','Вес (тонны)'),('vip','VIP')]:
    ...
    def distribute(self):
        ...
        results = self.company.optimize_cargo_distribution()
        
        # Обновляем представление транспорта
        self.refresh_vehicles()
        
        # Собираем результат для отображения
        distribution_result = []
        for v in self.company.vehicles:
            if v.current_load > 0:  # Показываем только используемый транспорт
                distribution_result.append({
                    'vehicle_id': str(v.vehicle_id),
                    'type': 'Поезд' if isinstance(v, Train) else 'Самолёт',
                    'capacity': v.capacity,
                    'current_load': v.current_load,
                    'clients': [{'name': c.name, 'cargo_weight': c.cargo_weight, 'vip': c.is_vip} for c in v.clients_list]
                })
        
        self.last_distribution = distribution_result
        self.show_distribution_modal(distribution_result, results)
        self.status('Распределение выполнено')

    def show_distribution_modal(self, distribution, results):
        ...
        tk.Label(info_frame, text=f"Всего клиентов: {len(self.company.clients)}", font=('Arial', 10)).pack(anchor='w')
        tk.Label(info_frame, text=f"Распределено: {len(results['distributed'])}", font=('Arial', 10)).pack(anchor='w')
        tk.Label(info_frame, text=f"Не распределено: {len(results['not_distributed'])}", font=('Arial', 10)).pack(anchor='w')
        tk.Label(info_frame, text=f"Использовано транспорта: {results['used_vehicles']}/{len(self.company.vehicles)}", font=('Arial', 10)).pack(anchor='w')

        # Таблица с результатами
        tree_frame = tk.Frame(dlg)
        tree_frame.pack(fill='both', expand=True, padx=10, pady=10)
        
        tree = ttk.Treeview(tree_frame, columns=('id','type','capacity','load','clients'), show='headings', height=10)
        ...
        for r in distribution:
            clients_str = '; '.join([f"{c['name']}({c['cargo_weight']}т)" for c in r['clients']])
        ...
        close_btn = tk.Button(btn_frame, text='Закрыть', command=dlg.destroy)
    ...
    def show_about(self):
        about_text = (
            'GUI для управления транспортной компанией\n'
            'Лабораторная работа №12 - Проектирование классов\n'
            'Вариант 5: Поезда и Самолеты'
        )
